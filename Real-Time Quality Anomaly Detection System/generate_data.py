...
SYSTEMS     = ["Blending", "Filling", "Lyophilization", "Capping", "Manufacturing", "QC Lab", "Warehouse"]
PRODUCTS    = ["Covovax", "Rota Vaccine", "BCG Freeze Dried", "BCG Immunotherapy", "EPO Vial", "EPO PFS", "Cy-Tb", "Blind Vaccine"]
...
ANOMALY_WINDOWS = [
    ...
    (datetime(2020,  9, 14), datetime(2020,  9, 21), "Blending",        3.8),  # Covovax bulk production start
    (datetime(2020, 11, 23), datetime(2020, 11, 30), "Lyophilization",  4.0),  # lyophilizer capacity issues
    ...
    for date in dates:
        ...
        for _ in range(np.random.poisson(rate)):
            ...
            rec = {
                ...
                "closed_timestamp": add_noise_missing(random_timestamp(date + timedelta(days=random.randint(3, 21))), 0.10),
    ...
    for date in dates:
        ...
        for _ in range(np.random.poisson(rate)):
            ...
            rec = {
                ...
                "resolution_days":  add_noise_missing(round(resolution_days(sev, "vendor_audit"), 1), 0.09),
                "closed_timestamp": add_noise_missing(random_timestamp(date + timedelta(days=random.randint(5, 60))), 0.12),
    ...
    for date in dates:
        ...
        for _ in range(np.random.poisson(rate)):
            ...
            rec = {
                ...
                "batch_id":         add_noise_missing(generate_batch_id(random.choice(PRODUCTS), date), 0.10),
                ...
                "closed_timestamp": add_noise_missing(random_timestamp(date + timedelta(days=random.randint(1, 7))), 0.10),
    ...
    for date in dates:
        ...
        for _ in range(np.random.poisson(base_rate)):
            ...
            rec = {
                ...
                "status":           random.choices(STATUSES, weights=[0.10, 0.75, 0.10, 0.05])[0],
                "resolution_days":  add_noise_missing(round(res, 1), 0.09),
                "closed_timestamp": add_noise_missing(random_timestamp(date + timedelta(days=res)), 0.13),
...
def main():
    ...
    print(f"COVID period rows : {df['is_covid_period'].sum():,} ({df['is_covid_period'].mean()*100:.1f}%)")
    print(f"\nMissing values:")
    missing = df.isnull().sum()
    print(missing[missing > 0].to_string())
    print(f"\nAnomaly windows   : {len(ANOMALY_WINDOWS)}")
    print(f"File size         : {round(df.memory_usage(deep=True).sum() / 1024**2, 1)} MB in memory")
