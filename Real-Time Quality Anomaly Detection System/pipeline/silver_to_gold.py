...
def engineer_features(df):
    ...
    df = df.withColumn(
        ...
        F.sum(
            F.when((F.col("event_type") == "capa") & (F.col("status") == "Overdue"), 1).otherwise(0)
        ).over(w30),
    )

    df = df.withColumn(
        "oos_count_7d", F.sum(F.when(F.col("event_type") == "oos", 1).otherwise(0)).over(w7)
    )

    df = df.withColumn(
        "critical_ratio_7d",
        F.sum(F.when(F.col("severity") == "Critical", 1).otherwise(0)).over(w7)
        / F.count("*").over(w7),
    )

    df = df.withColumn(
        "major_ratio_7d", F.sum(F.when(F.col("severity") == "Major", 1).otherwise(0)).over(w7)
    )

    df = df.withColumn(
        "unusual_event_count_7d",
        F.sum(
            F.when(F.col("event_type").isin("critical_alarm", "media_fill"), 1).otherwise(0)
        ).over(w7),
    )
    ...
    df = df.withColumn(
        ...
        F.sum(
            F.when(
                (F.col("event_type") == "complaint") & (F.col("root_cause") == "Material"), 1
            ).otherwise(0)
        ).over(w7),
    )
    ...
    df = df.withColumn(
        ...
        F.sum(
            F.when(
                (F.col("status") == "Open") | (F.col("status") == "Under Investigation"), 1
            ).otherwise(0)
        ).over(w7),
    )
...
def write_to_gold(df) -> None:
    ...
    (
        df.write.option("truncate", "true").jdbc(
            jdbc_url, "gold.qa_features", mode="overwrite", properties=properties
        )
    )
