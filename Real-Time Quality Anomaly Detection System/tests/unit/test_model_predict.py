...
class TestIsolationForestEvaluation:
    ...
    def test_anomaly_score_is_negated_decision_function(self, mock_iso_forest, fitted_scaler, sample_gold_df):
        from tests.conftest import FEATURE_COLS
        X = fitted_scaler.transform(sample_gold_df[FEATURE_COLS].values[:1])
        y = np.array([1])
...
class TestAutoencoderEvaluation:
    ...
    def test_zero_reconstruction_error_no_anomalies(self, mock_autoencoder, fitted_scaler, sample_gold_df):
        from tests.conftest import FEATURE_COLS
    ...
    def test_high_reconstruction_error_triggers_anomaly(self, mock_autoencoder, fitted_scaler, sample_gold_df):
        from tests.conftest import FEATURE_COLS
        X = fitted_scaler.transform(sample_gold_df[FEATURE_COLS].values[:1])
        y = np.array([1])
